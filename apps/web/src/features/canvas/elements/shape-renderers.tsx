...
export function renderElement(el: CanvasElement, theme: Theme): ReactNode {
  ...
  switch (el.type) {
    case 'rect':
      return <Rect width={w} height={h} fill={fill} cornerRadius={el.cornerRadius ?? 4} {...common} />;
    case 'ellipse':
      return <Ellipse x={w / 2} y={h / 2} radiusX={w / 2} radiusY={h / 2} fill={fill} {...common} />;
    case 'diamond':
      return <RegularPolygon x={w / 2} y={h / 2} sides={4} radius={Math.max(w, h) / 2} fill={fill} {...common} />;
    case 'triangle':
      return <RegularPolygon x={w / 2} y={h / 2} sides={3} radius={Math.max(w, h) / 2} fill={fill} {...common} />;
    case 'star':
      return (
        <Star
          x={w / 2}
